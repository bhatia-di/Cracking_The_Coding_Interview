...
* */
public class ExtractAllIntFromString {
    ...
    public int numDifferentIntegers(String word) {
        ...
        while (index<wordLen) {
            ...
            while(index < wordLen && Character.isDigit(word.charAt(index))) {
            ...
            if (temp.length() > 0) {
                ...
                track.add(removeZeroPrefix(temp));
                temp.setLength(0);
            }
    ...
    public String removeZeroPrefix(StringBuilder temp) {
        int index = 0;
        while(index < temp.length() && temp.charAt(index) == '0') {
            index++;
        }
        if (index == 0) return temp.toString();
        return temp.length() == 0 ? "0" : temp.delete(0, index).toString();
    }
}
